...
router.post('/', async (req, res) => {
  ...
  try {
    const id = uuid();
    const data = { id, name, url, status: 'active', stage: 1, created_at: now(), updated_at: now() };
...
// PATCH /api/projects/:id  { url?, name? }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Delete subcollections in a batch
...
router.post('/:id/direction', async (req, res) => {
  ...
  try {
    await db.collection('projects').doc(projectId)
      ...
      .set({ design_system, reference_urls, reference_notes, brand_assets, updated_at: now() }, { merge: true });
